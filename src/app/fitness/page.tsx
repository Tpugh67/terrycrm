...
const DEMO_DEALS = [
  { id: 1, title: "Personal Training - 6 Month", client: "John Smith", val: "$2,400", val2: "$400/mo", stage: "Inquiry", hot: true, follow: "Overdue" },
  { id: 2, title: "Group Fitness Membership", client: "Sarah Davis", val: "$1,200", val2: "$100/mo", stage: "Consultation", hot: false, follow: "Today" },
  { id: 3, title: "Nutrition Coaching", client: "Mike Johnson", val: "$1,800", val2: "$300/mo", stage: "Trial", hot: true, follow: "Tomorrow" },
  { id: 4, title: "Online Training Program", client: "Lisa Chen", val: "$997", val2: "$997 one-time", stage: "Enrolled", hot: false, follow: "Next week" },
  { id: 5, title: "Annual Membership", client: "Tom Wilson", val: "$960", val2: "$80/mo", stage: "Active", hot: false, follow: "Done" },

];

const STAGES = ["Inquiry", "Consultation", "Trial", "Enrolled", "Active"];
    ...
    <div className="min-h-screen bg-white">
      <nav className="border-b border-slate-200 px-6 py-4 flex items-center justify-between sticky top-0 bg-white z-50">
        ...
        <div className="flex items-center gap-3">
          ...
          <Link href="/login?mode=signup&industry=fitness" className="bg-green-600 hover:bg-green-700 text-white text-sm font-semibold px-4 py-2 rounded-lg transition">Start free trial</Link>
        </div>
      </nav>

      <section className="bg-slate-900 text-white py-20 px-6">
        <div className="max-w-4xl mx-auto text-center">
          <div className="inline-flex items-center gap-2 bg-white/10 text-white/80 text-xs font-semibold px-4 py-2 rounded-full mb-6 border border-white/20">💪 Gyms, Trainers & Wellness Coaches</div>
          <h1 className="text-4xl md:text-5xl font-bold mb-6 leading-tight">Your client pipeline,
built for fitness pros</h1>
          <p className="text-xl text-slate-400 max-w-2xl mx-auto mb-10">Track members from inquiry to enrolled, manage renewals, and grow your fitness business.</p>
          <Link href="/login?mode=signup&industry=fitness" className="inline-block bg-green-600 hover:bg-green-700 text-white font-bold text-lg px-10 py-4 rounded-xl transition">Start your free 14-day trial →</Link>
        ...
        <div className="max-w-6xl mx-auto">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
            {[
              ...
              { label: "Pipeline Value", value: `$${(totalVal/1000).toFixed(0)}K`, icon: "💰", color: "text-green-600" },
              ...
              <div key={deal.id} className="bg-white rounded-xl border border-slate-200 p-5 shadow-sm hover:shadow-md transition">
                <div className="flex items-start justify-between gap-4">
                  ...
                  <div className="text-right">
                    <div className="text-xs text-slate-400">Package Value</div>
                    <div className={`text-2xl font-bold text-green-600`}>{deal.val}</div>
        ...
        <div className="max-w-4xl mx-auto">
          ...
          <div className="grid md:grid-cols-2 gap-6">
              ...
              <div>
                ...
                <div className="text-sm text-slate-500 leading-relaxed">Track every prospect from inquiry to active member with fitness-specific stages.</div>
              </div>
            </div>
            <div className="flex gap-4 p-6 bg-slate-50 rounded-2xl border border-slate-100">
              <div className="text-3xl">💰</div>
              <div>
                <div className="font-bold text-slate-900 mb-1">Package Value Tracking</div>
                <div className="text-sm text-slate-500 leading-relaxed">Track package value, monthly rate, and revenue per client.</div>
              </div>
            </div>
            <div className="flex gap-4 p-6 bg-slate-50 rounded-2xl border border-slate-100">
              <div className="text-3xl">📅</div>
              <div>
                <div className="font-bold text-slate-900 mb-1">Renewal Reminders</div>
                <div className="text-sm text-slate-500 leading-relaxed">Set renewal dates and get alerts before members cancel.</div>
              </div>
            </div>
            <div className="flex gap-4 p-6 bg-slate-50 rounded-2xl border border-slate-100">
              <div className="text-3xl">📊</div>
              <div>
                <div className="font-bold text-slate-900 mb-1">Business Dashboard</div>
                <div className="text-sm text-slate-500 leading-relaxed">See total members, monthly revenue, and new signups at a glance.</div>
              </div>
            </div>
            <div className="flex gap-4 p-6 bg-slate-50 rounded-2xl border border-slate-100">
              <div className="text-3xl">🔥</div>
              <div>
                <div className="font-bold text-slate-900 mb-1">Hot Lead Alerts</div>
                <div className="text-sm text-slate-500 leading-relaxed">High-value package prospects get flagged so you close the big ones first.</div>
              </div>
            </div>
            <div className="flex gap-4 p-6 bg-slate-50 rounded-2xl border border-slate-100">
              <div className="text-3xl">📞</div>
              <div>
                ...
                <div className="text-sm text-slate-500 leading-relaxed">Log every call, session, and check-in with one click.</div>
              </div>
            </div>
      ...
      <section className="bg-slate-900 py-20 px-6">
        <div className="max-w-2xl mx-auto text-center">
          <h2 className="text-3xl font-bold text-white mb-4">Ready to grow your fitness business?</h2>
          <p className="text-slate-400 text-lg mb-8">Start your free 14-day trial. No credit card required.</p>
          <Link href="/login?mode=signup&industry=fitness" className="inline-block bg-green-600 hover:bg-green-700 text-white font-bold text-lg px-10 py-4 rounded-xl transition">Start free trial →</Link>
