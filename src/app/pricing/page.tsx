...
const PLANS = [
  ...
  { name: "Team", price: "$59", period: "/month", description: "For small teams closing more deals", priceId: "price_1TT1CDANf6sspitdK6JFF3uq", features: ["Up to 5 users","All 18 industry pipelines","Unlimited deals & contacts","Follow-up reminders","Hot deal alerts","Activity timeline","Priority support","Team activity feed"], popular: true },
];
          ...
          {PLANS.map((plan) => (
            <div key={plan.name} className={"bg-white rounded-2xl border-2 p-8 relative " + (plan.popular ? "border-blue-500 shadow-xl" : "border-slate-200 shadow-sm")}>
              ...
              <ul className="space-y-3 mb-8">
                {plan.features.map((f) => (
                  <li key={f} className="flex items-center gap-3 text-sm text-slate-600"><span className="text-emerald-500 font-bold">✓</span>{f}</li>
                ))}
              </ul>
