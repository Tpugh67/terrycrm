...
const DEMO_DEALS = [
  { id: 1, title: "Retirement Planning - Johnson", client: "Bob Johnson", val: "$850,000", val2: "Fee: $8,500/yr", stage: "Prospect", hot: true, follow: "Overdue" },
  { id: 2, title: "College Fund Setup", client: "Sarah Williams", val: "$250,000", val2: "Fee: $2,500/yr", stage: "Initial Meeting", hot: true, follow: "Today" },
  { id: 3, title: "Estate Planning", client: "Mike Davis", val: "$1,200,000", val2: "Fee: $12,000/yr", stage: "Proposal", hot: false, follow: "Tomorrow" },
  { id: 4, title: "401K Rollover", client: "Lisa Chen", val: "$425,000", val2: "Fee: $4,250/yr", stage: "Compliance Review", hot: true, follow: "Next week" },
  { id: 5, title: "Investment Portfolio", client: "Tom Wilson", val: "$650,000", val2: "Fee: $6,500/yr", stage: "Active Client", hot: false, follow: "Done" },

];

const STAGES = ["Prospect", "Initial Meeting", "Proposal", "Compliance Review", "Active Client"];
    ...
    <div className="min-h-screen bg-white">
      <nav className="border-b border-slate-200 px-6 py-4 flex items-center justify-between sticky top-0 bg-white z-50">
        ...
        <div className="flex items-center gap-3">
          ...
          <Link href="/login?mode=signup&industry=financial" className="bg-blue-700 hover:bg-blue-800 text-white text-sm font-semibold px-4 py-2 rounded-lg transition">Start free trial</Link>
        </div>
      </nav>

      <section className="bg-slate-900 text-white py-20 px-6">
        <div className="max-w-4xl mx-auto text-center">
          <div className="inline-flex items-center gap-2 bg-white/10 text-white/80 text-xs font-semibold px-4 py-2 rounded-full mb-6 border border-white/20">📈 Financial Advisors & Planners</div>
          <h1 className="text-4xl md:text-5xl font-bold mb-6 leading-tight">Your client pipeline,
built for advisors</h1>
          <p className="text-xl text-slate-400 max-w-2xl mx-auto mb-10">Track prospects from first meeting to active client, manage AUM, and grow your book of business.</p>
          <Link href="/login?mode=signup&industry=financial" className="inline-block bg-blue-700 hover:bg-blue-800 text-white font-bold text-lg px-10 py-4 rounded-xl transition">Start your free 14-day trial →</Link>
      ...
      <section className="py-16 px-6 bg-slate-50">
        <div className="max-w-6xl mx-auto">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
            {[
              { label: "Active Clients", value: String(DEMO_DEALS.length), icon: "📈", color: "text-slate-900" },
              { label: "Pipeline Value", value: `$${(totalVal/1000).toFixed(0)}K`, icon: "💰", color: "text-blue-700" },
      ...
      <section className="py-20 px-6">
        <div className="max-w-4xl mx-auto">
          ...
          <div className="grid md:grid-cols-2 gap-6">
              ...
              <div>
                <div className="font-bold text-slate-900 mb-1">Client Pipeline</div>
                <div className="text-sm text-slate-500 leading-relaxed">Track every prospect from first meeting to active client with advisor-specific stages.</div>
              </div>
            </div>
            <div className="flex gap-4 p-6 bg-slate-50 rounded-2xl border border-slate-100">
              <div className="text-3xl">💰</div>
              <div>
                <div className="font-bold text-slate-900 mb-1">AUM Tracking</div>
                <div className="text-sm text-slate-500 leading-relaxed">Track assets under management, annual fees, and revenue per client.</div>
              </div>
            </div>
            <div className="flex gap-4 p-6 bg-slate-50 rounded-2xl border border-slate-100">
              <div className="text-3xl">📅</div>
              <div>
                <div className="font-bold text-slate-900 mb-1">Review Date Alerts</div>
                <div className="text-sm text-slate-500 leading-relaxed">Set annual review dates and get alerts so no client goes neglected.</div>
              </div>
            </div>
            <div className="flex gap-4 p-6 bg-slate-50 rounded-2xl border border-slate-100">
              <div className="text-3xl">📊</div>
              <div>
                <div className="font-bold text-slate-900 mb-1">Book of Business Dashboard</div>
                <div className="text-sm text-slate-500 leading-relaxed">See total AUM, annual revenue, and new clients this month.</div>
              </div>
            </div>
            <div className="flex gap-4 p-6 bg-slate-50 rounded-2xl border border-slate-100">
              <div className="text-3xl">🔥</div>
              <div>
                <div className="font-bold text-slate-900 mb-1">High-Value Client Alerts</div>
                <div className="text-sm text-slate-500 leading-relaxed">Large AUM prospects get flagged so you prioritize the right opportunities.</div>
              </div>
            </div>
            <div className="flex gap-4 p-6 bg-slate-50 rounded-2xl border border-slate-100">
              <div className="text-3xl">📞</div>
              <div>
                ...
                <div className="text-sm text-slate-500 leading-relaxed">Log every meeting, call, and recommendation with one click.</div>
              </div>
            </div>
      ...
      <section className="bg-slate-900 py-20 px-6">
        <div className="max-w-2xl mx-auto text-center">
          ...
          <Link href="/login?mode=signup&industry=financial" className="inline-block bg-blue-700 hover:bg-blue-800 text-white font-bold text-lg px-10 py-4 rounded-xl transition">Start free trial →</Link>
