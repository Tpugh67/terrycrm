...
const STAGES = ["New Inquiry", "Consultation", "Enrolled", "Active Student", "Graduating", "Alumni"];
const STAGE_DOT: Record<string,string> = { "New Inquiry":"bg-indigo-500","Consultation":"bg-violet-500","Enrolled":"bg-blue-500","Active Student":"bg-emerald-500","Graduating":"bg-amber-500","Alumni":"bg-slate-500" };
const EMPTY_FORM = { title:"",address:"",arv:"",offer:"",seller:"",amount:"",stage:"New Inquiry",contact_email:"",next_follow_up:"" };
...
export default function EducationPipeline(){
  ...
  function openAdd(stage="New Inquiry"){setForm({...EMPTY_FORM,stage});setEditId(null);setPanelOpen(true);}
  function openEdit(deal:Deal){setForm({title:deal.title||"",address:deal.address||"",arv:deal.arv||"",offer:deal.offer||"",seller:deal.seller||"",amount:deal.amount||"",stage:deal.stage||"New Inquiry",contact_email:deal.contact_email||"",next_follow_up:deal.next_follow_up||""});setEditId(deal.id??null);setPanelOpen(true);}
  ...
  async function handleSubmit(e:React.FormEvent){e.preventDefault();if(!form.title||!form.seller)return;setSaving(true);const user=await getCurrentUser();if(!user){setSaving(false);return;}
    if(editId!==null){const{error}=await supabase.from("deals").update({...form,next_follow_up:form.next_follow_up||null,updated_at:new Date().toISOString()}).eq("id",editId).eq("user_id",user.id);if(error){alert(error.message);setSaving(false);return;}await supabase.from("notes").insert({deal_id:editId,user_id:user.id,content:"✏️ Student updated"});}
  ...
  async function quickAction(dealId:number,action:"call"|"email"|"session"|"progress"){const map={call:"📞 Called student",email:"📧 Sent email",session:"📚 Session completed",progress:"📊 Progress update sent"};await addNote(dealId,map[action]);}
  const filteredDeals=useMemo(()=>{let r=deals;if(search){const q=search.toLowerCase();r=r.filter((d)=>[d.title,d.seller,d.address].join(" ").toLowerCase().includes(q));}if(filter==="hot")r=r.filter((d)=>parseMoney(d.arv)>=5000);if(filter==="overdue")r=r.filter((d)=>isOverdue(d.next_follow_up));if(filter==="stale")r=r.filter((d)=>isStale(d));return r;},[deals,search,filter]);
  ...
  const activeStudents=deals.filter((d)=>d.stage==="Active Student").length;
  const totalRevenue=deals.filter((d)=>["Active Student","Graduating","Alumni"].includes(d.stage)).reduce((s,d)=>s+parseMoney(d.arv),0);
  ...
  return(
    <div className="min-h-screen bg-slate-50 -m-8">
      <div className="bg-white border-b border-slate-200 px-6 py-3 flex items-center justify-between sticky top-0 z-30">
        <div className="flex items-center gap-3"><div className="w-7 h-7 rounded-lg bg-indigo-600 flex items-center justify-center text-white text-xs font-bold">ED</div><h1 className="text-base font-bold text-slate-900">Education Pipeline</h1><span className="text-slate-300">·</span><span className="text-sm text-slate-400">{deals.length} students</span></div>
        <div className="flex items-center gap-2"><input value={search} onChange={(e)=>setSearch(e.target.value)} placeholder="Search students..." className="text-sm border border-slate-200 rounded-lg px-3 py-2 w-48 bg-slate-50 focus:outline-none focus:ring-2 focus:ring-indigo-600"/><button onClick={()=>openAdd()} className="bg-indigo-600 hover:bg-indigo-700 text-white text-sm font-semibold px-4 py-2 rounded-lg transition">+ Add Student</button></div>
      </div>
      <div className="bg-white border-b border-slate-200 px-6 py-4 grid grid-cols-2 md:grid-cols-4 gap-6">
        {[{label:"Pipeline Value",value:fmtShort(totalPipeline),sub:`${deals.length} total students`},{label:"Revenue Earned",value:fmtShort(totalRevenue),sub:"enrolled students",green:true},{label:"Active Students",value:String(activeStudents),sub:"currently enrolled",green:activeStudents>0},{label:"Follow-ups Due",value:String(overdueCount),sub:overdueCount>0?"overdue":"all clear",red:overdueCount>0}].map((s)=>(<div key={s.label}><div className="text-xs font-semibold text-slate-400 uppercase tracking-wider">{s.label}</div><div className={`text-2xl font-bold mt-1 ${(s as any).red?"text-red-500":(s as any).green?"text-emerald-600":"text-slate-900"}`}>{s.value}</div><div className={`text-xs mt-0.5 ${(s as any).red?"text-red-400":"text-slate-400"}`}>{s.sub}</div></div>))}
      </div>
      <div className="px-6 py-3 flex items-center gap-2 flex-wrap">
        {(["all","hot","overdue","stale"] as Filter[]).map((f)=>(<button key={f} onClick={()=>setFilter(f)} className={`text-xs px-3 py-1.5 rounded-full border font-medium transition ${filter===f?"bg-indigo-600 text-white border-indigo-600":"bg-white text-slate-500 border-slate-200"}`}>{f==="all"&&"All students"}{f==="hot"&&"📚 High value ($5k+)"}{f==="overdue"&&"⚠️ Overdue"}{f==="stale"&&"⏳ Stale 7d+"}</button>))}
          ...
          {STAGES.map((stage)=>{const items=stageDeals(stage);const val=stageVal(stage);return(
            <div key={stage} className="flex flex-col">
              ...
              <div className="flex flex-col gap-3">
                {items.length===0&&<div className="border-2 border-dashed border-slate-200 rounded-xl flex items-center justify-center h-20 text-xs text-slate-400">No students</div>}
                {items.map((d)=>{const contact=getContact(d.contact_email);const hot=parseMoney(d.arv)>=5000;const stale=isStale(d);const overdue=isOverdue(d.next_follow_up);const dueToday=isDueToday(d.next_follow_up);const expanded=expandedDeal===d.id;const dealNotes=getDealNotes(d.id);return(
                  <div key={d.id} className={`bg-white rounded-xl border text-sm shadow-sm hover:shadow-md transition-all ${hot?"border-indigo-200":"border-slate-200"}`}>
                    <div className="p-4 cursor-pointer select-none" onClick={()=>setExpandedDeal(expanded?null:d.id!)}>
                      <div className="flex flex-wrap gap-1 mb-2">{hot&&<span className="text-[10px] px-2 py-0.5 rounded-full bg-indigo-100 text-indigo-800 font-semibold">📚 High Value</span>}{stale&&<span className="text-[10px] px-2 py-0.5 rounded-full bg-slate-100 text-slate-500">⏳ Stale</span>}{overdue&&<span className="text-[10px] px-2 py-0.5 rounded-full bg-red-100 text-red-600 font-semibold">⚠️ Overdue</span>}</div>
                      ...
                      <div className="mt-3 grid grid-cols-2 gap-x-4 gap-y-1.5">{[["Program Value",fmt(d.arv)],["Course / Program",d.offer||"—"],["Monthly Fee",fmt(d.amount)],["Student",d.seller]].map(([label,val])=>(<div key={label}><div className="text-[10px] text-slate-400 uppercase tracking-wide font-semibold">{label}</div><div className="text-xs font-bold text-slate-800 truncate">{val}</div></div>))}</div>
                    ...
                    {expanded&&(<div className="border-t border-slate-100 px-4 pb-4 pt-3">
                      ...
                      <div className="grid grid-cols-2 gap-2 mb-4">{(["call","email","session","progress"] as const).map((a)=>(<button key={a} onClick={()=>quickAction(d.id!,a)} className="text-[11px] py-1.5 border border-slate-200 rounded-lg bg-slate-50 hover:bg-slate-100 text-slate-600 font-medium">{a==="call"&&"📞 Call"}{a==="email"&&"📧 Email"}{a==="session"&&"📚 Session"}{a==="progress"&&"📊 Progress"}</button>))}</div>
                ...
                <button onClick={()=>openAdd(stage)} className="text-xs text-slate-400 hover:text-indigo-600 border-2 border-dashed border-slate-200 hover:border-indigo-300 rounded-xl py-3 transition">+ Add student</button>
              </div>
            </div>);})}
        </div>
      )}
      {panelOpen&&(<div className="fixed inset-0 z-50 flex"><div className="flex-1 bg-black/30" onClick={closePanel}/>
        <div className="w-full max-w-md bg-white h-full shadow-2xl overflow-y-auto flex flex-col">
          <div className="flex items-center justify-between px-6 py-4 border-b border-slate-200 sticky top-0 bg-white"><div className="flex items-center gap-2"><div className="w-6 h-6 rounded bg-indigo-600 flex items-center justify-center text-white text-[10px] font-bold">ED</div><h2 className="text-base font-bold text-slate-900">{editId!==null?"Edit Student":"New Student"}</h2></div><button onClick={closePanel} className="text-slate-400 text-2xl">x</button></div>
          <form onSubmit={handleSubmit} className="flex-1 px-6 py-5 space-y-4">
            {[{label:"Program / Course Name *",name:"title",placeholder:"e.g. Johnson - SAT Prep Program",required:true},{label:"Student Name *",name:"seller",placeholder:"Student full name",required:true},{label:"School / Grade Level",name:"address",placeholder:"e.g. Lincoln High School, Grade 11"}].map((f)=>(<div key={f.name}><label className="block text-xs font-bold text-slate-500 uppercase tracking-wide mb-1">{f.label}</label><input name={f.name} value={(form as any)[f.name]} onChange={handleChange} required={f.required} placeholder={f.placeholder} className="w-full border border-slate-200 rounded-lg px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-600"/></div>))}
            <div className="grid grid-cols-2 gap-3"><div><label className="block text-xs font-bold text-slate-500 uppercase tracking-wide mb-1">Program Value ($)</label><input name="arv" value={form.arv} onChange={handleChange} placeholder="e.g. 3600" className="w-full border border-slate-200 rounded-lg px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-600"/></div><div><label className="block text-xs font-bold text-slate-500 uppercase tracking-wide mb-1">Course / Program</label><input name="offer" value={form.offer} onChange={handleChange} placeholder="e.g. SAT Prep, Coding" className="w-full border border-slate-200 rounded-lg px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-600"/></div></div>
            <div><label className="block text-xs font-bold text-slate-500 uppercase tracking-wide mb-1">Monthly Fee ($)</label><input name="amount" value={form.amount} onChange={handleChange} placeholder="e.g. 300" className="w-full border border-slate-200 rounded-lg px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-600"/></div>
            ...
            <div className="flex gap-3 pt-2 pb-6"><button type="submit" disabled={saving} className="flex-1 bg-indigo-600 hover:bg-indigo-700 disabled:opacity-50 text-white font-bold text-sm py-3 rounded-lg">{saving?"Saving...":editId!==null?"Update Student":"Add Student"}</button><button type="button" onClick={closePanel} className="px-5 border border-slate-200 text-slate-600 text-sm py-3 rounded-lg">Cancel</button></div>
          </form>
        </div>
      </div>)}
    </div>
