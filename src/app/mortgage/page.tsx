...
const DEMO_DEALS = [
  { id: 1, title: "Anderson Purchase — FHA 3.5%", client: "Mark Anderson", loanAmt: "$320,000", commission: "$3,200", stage: "New Lead", hot: true, followUp: "Overdue" },
  { id: 2, title: "Garcia Refinance — Conv 30yr", client: "Maria Garcia", loanAmt: "$485,000", commission: "$4,850", stage: "Pre-Qualified", hot: true, followUp: "Today" },
  { id: 3, title: "Lee Investment Property", client: "Kevin Lee", loanAmt: "$650,000", commission: "$6,500", stage: "Application", hot: false, followUp: "Tomorrow" },
  { id: 4, title: "Patel First-Time Buyer", client: "Priya Patel", loanAmt: "$290,000", commission: "$2,900", stage: "Processing", hot: true, followUp: "Next week" },
  { id: 5, title: "Robinson Cash-Out Refi", client: "Angela Robinson", loanAmt: "$410,000", commission: "$4,100", stage: "Closed", hot: false, followUp: "Done" },
];

const STAGES = ["New Lead", "Pre-Qualified", "Application", "Processing", "Closed"];
    ...
    <div className="min-h-screen bg-white">
      <nav className="border-b border-slate-200 px-6 py-4 flex items-center justify-between sticky top-0 bg-white z-50">
        ...
        <div className="flex items-center gap-3">
          ...
          <Link href="/login?mode=signup&industry=mortgage" className="bg-blue-600 hover:bg-blue-700 text-white text-sm font-semibold px-4 py-2 rounded-lg transition">Start free trial</Link>
        </div>
      </nav>
      <section className="bg-slate-900 text-white py-20 px-6">
        <div className="max-w-4xl mx-auto text-center">
          <div className="inline-flex items-center gap-2 bg-violet-600/20 text-violet-400 text-xs font-semibold px-4 py-2 rounded-full mb-6 border border-violet-600/30">🏦 Mortgage Loan Officers & Brokers</div>
          <h1 className="text-4xl md:text-5xl font-bold mb-6 leading-tight">Your loan pipeline,<br />built for mortgage pros</h1>
          <p className="text-xl text-slate-400 max-w-2xl mx-auto mb-10">Track borrowers from pre-qual to closing. Log every document, call, and condition. Built for loan officers — not generic salespeople.</p>
          <Link href="/login?mode=signup&industry=mortgage" className="inline-block bg-blue-600 hover:bg-blue-700 text-white font-bold text-lg px-10 py-4 rounded-xl transition">Start your free 14-day trial →</Link>
        ...
        <div className="max-w-6xl mx-auto">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
            {[
              ...
              { label: "Hot Borrowers", value: String(hotDeals), icon: "🔥", color: "text-red-500" },
              ...
              <div key={deal.id} className="bg-white rounded-xl border border-slate-200 p-5 shadow-sm hover:shadow-md transition">
                <div className="flex items-start justify-between gap-4">
                  ...
                  <div className="text-right">
                    ...
                    <div className="text-xs text-slate-400 mt-1">Loan: {deal.loanAmt}</div>
                  </div>
                </div>
                <div className="mt-3 pt-3 border-t border-slate-100 flex items-center justify-between">
                  ...
                  <div className="flex gap-2">
                    ...
                    <button className="text-xs bg-blue-50 hover:bg-blue-100 text-blue-600 px-3 py-1.5 rounded-lg transition">📄 Log Doc</button>
                  </div>
                </div>
              </div>
            ))}
      ...
      <section className="bg-slate-900 py-20 px-6">
        <div className="max-w-2xl mx-auto text-center">
          ...
          <Link href="/login?mode=signup&industry=mortgage" className="inline-block bg-blue-600 hover:bg-blue-700 text-white font-bold text-lg px-10 py-4 rounded-xl transition">Start free trial →</Link>
