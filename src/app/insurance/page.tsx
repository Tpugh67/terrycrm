...
const STAGES = ["New Lead", "Quoted", "Proposal Sent", "Negotiating", "Closed Won", "Closed Lost"];
...
export default function InsurancePipeline() {
  const config = getIndustryConfig("insurance");
  ...
  function openEdit(deal: Deal) {
    setForm({ title: deal.title || "", address: deal.address || "", arv: deal.arv || "", offer: deal.offer || "",
      seller: deal.seller || "", amount: deal.amount || "", stage: deal.stage || "New Lead",
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (editId !== null) {
      ...
      await supabase.from("notes").insert({ deal_id: editId, user_id: user.id, content: "✏️ Policy updated" });
    } else {
  ...
  async function quickAction(dealId: number, action: "call" | "text" | "quote" | "renewal") {
    const map = { call: "📞 Called client", text: "💬 Sent text", quote: "📄 Sent quote", renewal: "🔄 Renewal reminder sent" };
  ...
  const filteredDeals = useMemo(() => {
    ...
    if (filter === "hot") r = r.filter((d) => parseMoney(d.amount) >= 5000);
  ...
  const stagePremium = (s: string) => stageDeals(s).reduce((sum, d) => sum + parseMoney(d.amount), 0);
  const totalPremium = deals.reduce((s, d) => s + parseMoney(d.amount), 0);
  const closedPremium = deals.filter((d) => d.stage === "Closed Won").reduce((s, d) => s + parseMoney(d.amount), 0);
  const overdueCount = deals.filter((d) => isOverdue(d.next_follow_up)).length;
  const conversionRate = deals.length > 0 ? Math.round((deals.filter((d) => d.stage === "Closed Won").length / deals.length) * 100) : 0;
    ...
    <div className="min-h-screen bg-slate-50 -m-8">
      ...
      <div className="bg-white border-b border-slate-200 px-6 py-3 flex items-center justify-between sticky top-0 z-30">
        <div className="flex items-center gap-3">
          <div className="w-7 h-7 rounded-lg bg-emerald-600 flex items-center justify-center text-white text-xs font-bold">IN</div>
          <h1 className="text-base font-bold text-slate-900">Insurance Pipeline</h1>
          <span className="text-slate-300">·</span>
          <span className="text-sm text-slate-400">{deals.length} policies</span>
        </div>
        <div className="flex items-center gap-2">
          <input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Search policies..."
            className="text-sm border border-slate-200 rounded-lg px-3 py-2 w-48 bg-slate-50 focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:bg-white" />
          <button onClick={() => openAdd()} className="bg-emerald-600 hover:bg-emerald-700 text-white text-sm font-semibold px-4 py-2 rounded-lg transition flex items-center gap-1">
            <span className="text-lg leading-none">+</span> New Policy
          </button>
        </div>
      </div>
      ...
      <div className="bg-white border-b border-slate-200 px-6 py-4 grid grid-cols-2 md:grid-cols-4 gap-6">
        {[
          { label: "Pipeline Premium", value: fmtShort(totalPremium), sub: `${deals.length} policies` },
          { label: "Closed Premium", value: fmtShort(closedPremium), sub: "won this period", green: true },
          { label: "Conversion Rate", value: `${conversionRate}%`, sub: "leads to closed", green: conversionRate > 30 },
      ...
      <div className="px-6 py-3 flex items-center gap-2 flex-wrap">
        {(["all", "hot", "overdue", "stale"] as Filter[]).map((f) => (
          <button key={f} onClick={() => setFilter(f)}
            className={`text-xs px-3 py-1.5 rounded-full border font-medium transition ${filter === f ? "bg-emerald-600 text-white border-emerald-600" : "bg-white text-slate-500 border-slate-200 hover:border-slate-300"}`}>
            {f === "all" && "All policies"}{f === "hot" && "⭐ High value"}{f === "overdue" && "⚠️ Overdue"}{f === "stale" && "⏳ Stale 7d+"}
          </button>
        ))}
      ...
      {loading ? (
        ...
        <div className="px-6 pb-12 grid grid-cols-1 md:grid-cols-3 xl:grid-cols-6 gap-4">
          {STAGES.map((stage) => {
            ...
            const premium = stagePremium(stage);
            ...
            return (
              <div key={stage} className="flex flex-col">
                <div className="flex items-center justify-between mb-3">
                  ...
                  {premium > 0 && <span className="text-xs text-slate-400">{fmtShort(premium)}</span>}
                </div>
                <div className="flex flex-col gap-3">
                  {items.length === 0 && (
                    <div className="border-2 border-dashed border-slate-200 rounded-xl flex items-center justify-center h-20 text-xs text-slate-400">No policies</div>
                  )}
                  {items.map((d) => {
                    const contact = getContact(d.contact_email);
                    const premium = parseMoney(d.amount);
                    const highValue = premium >= 5000;
                    ...
                    return (
                      <div key={d.id} className={`bg-white rounded-xl border text-sm shadow-sm hover:shadow-md transition-all ${isLost ? "opacity-60" : ""} ${highValue ? "border-emerald-200" : "border-slate-200"}`}>
                        <div className="p-4 cursor-pointer select-none" onClick={() => setExpandedDeal(expanded ? null : d.id!)}>
                          <div className="flex flex-wrap gap-1 mb-2">
                            {highValue && !isLost && <span className="text-[10px] px-2 py-0.5 rounded-full bg-emerald-100 text-emerald-800 font-semibold">⭐ High Value</span>}
                          ...
                          {d.address && <div className="text-xs text-slate-500 mt-0.5">{d.address}</div>}
                          <div className="mt-3 space-y-1.5">
                            <div>
                              <div className="text-[10px] text-slate-400 uppercase tracking-wide font-semibold">Annual Premium</div>
                              <div className={`text-sm font-bold ${highValue ? "text-emerald-600" : "text-slate-800"}`}>{fmt(d.amount)}</div>
                            </div>
                            {d.arv && <div>
                              <div className="text-[10px] text-slate-400 uppercase tracking-wide font-semibold">Coverage</div>
                              <div className="text-xs text-slate-700">{d.arv}</div>
                            </div>}
                          </div>
                          {d.next_follow_up && (
                            <div className={`mt-2 text-xs font-medium ${overdue ? "text-red-500" : dueToday ? "text-amber-500" : "text-slate-400"}`}>
                              {overdue ? "⚠️ Renewal overdue · " : dueToday ? "📅 Due today · " : "📅 "}{fmtDate(d.next_follow_up)}
                            </div>
                          )}
                          <div className="mt-3 flex items-center justify-between">
                            {contact ? (
                              <div className="flex items-center gap-2">
                                <div className="w-6 h-6 rounded-full bg-emerald-100 text-emerald-700 text-[9px] font-bold flex items-center justify-center">{initials(contact.name)}</div>
                        ...
                        {expanded && (
                          <div className="border-t border-slate-100 px-4 pb-4 pt-3">
                            <select value={d.stage} onChange={(e) => handleStageChange(d.id, e.target.value)}
                              className="w-full text-xs border border-slate-200 rounded-lg px-3 py-2 bg-white mb-3 focus:outline-none focus:ring-2 focus:ring-emerald-500">
                              {STAGES.map((s) => <option key={s}>{s}</option>)}
                            </select>
                            <div className="grid grid-cols-2 gap-2 mb-4">
                              {(["call", "text", "quote", "renewal"] as const).map((a) => (
                                <button key={a} onClick={() => quickAction(d.id!, a)}
                                  ...
                                  {a === "call" && "📞 Call"}{a === "text" && "💬 Text"}{a === "quote" && "📄 Quote"}{a === "renewal" && "🔄 Renewal"}
                                </button>
                              ))}
                            </div>
                            ...
                            <input value={noteInputs[d.id!] || ""} onChange={(e) => setNoteInputs((p) => ({ ...p, [d.id!]: e.target.value }))}
                              ...
                              className="w-full text-xs border border-slate-200 rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-emerald-500" />
                            <div className="flex gap-4 mt-3">
                              <button onClick={() => { openEdit(d); setExpandedDeal(null); }} className="text-xs text-emerald-600 hover:text-emerald-800 font-semibold">Edit</button>
                  ...
                  {!isLost && (
                    <button onClick={() => openAdd(stage)} className="text-xs text-slate-400 hover:text-emerald-600 border-2 border-dashed border-slate-200 hover:border-emerald-300 rounded-xl py-3 transition">
                      + Add policy
                    </button>
                  )}
                </div>
        ...
        <div className="fixed inset-0 z-50 flex">
          ...
          <div className="w-full max-w-md bg-white h-full shadow-2xl overflow-y-auto flex flex-col">
            <div className="flex items-center justify-between px-6 py-4 border-b border-slate-200 sticky top-0 bg-white z-10">
              <div className="flex items-center gap-2">
                <div className="w-6 h-6 rounded bg-emerald-600 flex items-center justify-center text-white text-[10px] font-bold">IN</div>
                <h2 className="text-base font-bold text-slate-900">{editId !== null ? "Edit Policy" : "New Insurance Policy"}</h2>
              </div>
            ...
            <form onSubmit={handleSubmit} className="flex-1 px-6 py-5 space-y-4">
              <div>
                <label className="block text-xs font-bold text-slate-500 uppercase tracking-wide mb-1">Policy Name *</label>
                <input name="title" value={form.title} onChange={handleChange} required placeholder="e.g. Smith Family Home Policy"
              ...
              <div>
                ...
                <input name="seller" value={form.seller} onChange={handleChange} required placeholder="Client full name"
                  className="w-full border border-slate-200 rounded-lg px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-emerald-500" />
              </div>
              <div>
                <label className="block text-xs font-bold text-slate-500 uppercase tracking-wide mb-1">Policy Type</label>
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block text-xs font-bold text-slate-500 uppercase tracking-wide mb-1">Coverage Amount</label>
                  <input name="arv" value={form.arv} onChange={handleChange} placeholder="e.g. $500,000"
                    className="w-full border border-slate-200 rounded-lg px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-emerald-500" />
                </div>
                <div>
                  <label className="block text-xs font-bold text-slate-500 uppercase tracking-wide mb-1">Annual Premium ($)</label>
                  <input name="amount" value={form.amount} onChange={handleChange} placeholder="e.g. 3500"
                    className="w-full border border-slate-200 rounded-lg px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-emerald-500" />
                </div>
              </div>
              <div className="grid grid-cols-2 gap-3">
                ...
                <div>
                  <label className="block text-xs font-bold text-slate-500 uppercase tracking-wide mb-1">Renewal / Follow-up</label>
              ...
              <div className="flex gap-3 pt-2 pb-6">
                <button type="submit" disabled={saving}
                  className="flex-1 bg-emerald-600 hover:bg-emerald-700 disabled:opacity-50 text-white font-bold text-sm py-3 rounded-lg transition">
                  {saving ? "Saving..." : editId !== null ? "Update Policy" : "Create Policy"}
                </button>
