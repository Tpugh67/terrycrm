...
const STAGES = ["New Inquiry", "Test Drive", "Negotiating", "Financing", "Closed Won", "Closed Lost"];
const STAGE_DOT: Record<string,string> = { "New Inquiry":"bg-red-500","Test Drive":"bg-orange-500","Negotiating":"bg-amber-500","Financing":"bg-blue-500","Closed Won":"bg-emerald-500","Closed Lost":"bg-slate-400" };
...
export default function AutoPipeline(){
  ...
  async function handleSubmit(e:React.FormEvent){e.preventDefault();if(!form.title||!form.seller)return;setSaving(true);const user=await getCurrentUser();if(!user){setSaving(false);return;}
    if(editId!==null){const{error}=await supabase.from("deals").update({...form,next_follow_up:form.next_follow_up||null,updated_at:new Date().toISOString()}).eq("id",editId).eq("user_id",user.id);if(error){alert(error.message);setSaving(false);return;}await supabase.from("notes").insert({deal_id:editId,user_id:user.id,content:"✏️ Deal updated"});}
  ...
  async function quickAction(dealId:number,action:"call"|"text"|"testdrive"|"offer"){const map={call:"📞 Called customer",text:"💬 Sent text",testdrive:"🚗 Test drive scheduled",offer:"💰 Made offer"};await addNote(dealId,map[action]);}
  ...
  const totalCommission=deals.filter((d)=>d.stage==="Closed Won").reduce((s,d)=>s+parseMoney(d.amount),0);
  ...
  return(
    <div className="min-h-screen bg-slate-50 -m-8">
      <div className="bg-white border-b border-slate-200 px-6 py-3 flex items-center justify-between sticky top-0 z-30">
        <div className="flex items-center gap-3"><div className="w-7 h-7 rounded-lg bg-red-600 flex items-center justify-center text-white text-xs font-bold">AU</div><h1 className="text-base font-bold text-slate-900">Auto Sales Pipeline</h1><span className="text-slate-300">·</span><span className="text-sm text-slate-400">{deals.length} deals</span></div>
        <div className="flex items-center gap-2"><input value={search} onChange={(e)=>setSearch(e.target.value)} placeholder="Search..." className="text-sm border border-slate-200 rounded-lg px-3 py-2 w-48 bg-slate-50 focus:outline-none focus:ring-2 focus:ring-red-500"/><button onClick={()=>openAdd()} className="bg-red-600 hover:bg-red-700 text-white text-sm font-semibold px-4 py-2 rounded-lg transition">+ Add Deal</button></div>
      </div>
      <div className="bg-white border-b border-slate-200 px-6 py-4 grid grid-cols-2 md:grid-cols-4 gap-6">
        {[{label:"Pipeline Value",value:fmtShort(totalPipeline),sub:`${deals.length} active deals`},{label:"Closed Revenue",value:fmtShort(closedRevenue),sub:"vehicles sold",green:true},{label:"Commission Earned",value:fmtShort(totalCommission),sub:"from closed deals",green:totalCommission>0},{label:"Follow-ups Due",value:String(overdueCount),sub:overdueCount>0?"overdue":"all clear ✓",red:overdueCount>0}].map((s)=>(<div key={s.label}><div className="text-xs font-semibold text-slate-400 uppercase tracking-wider">{s.label}</div><div className={`text-2xl font-bold mt-1 ${(s as any).red?"text-red-500":(s as any).green?"text-emerald-600":"text-slate-900"}`}>{s.value}</div><div className={`text-xs mt-0.5 ${(s as any).red?"text-red-400":"text-slate-400"}`}>{s.sub}</div></div>))}
      </div>
      <div className="px-6 py-3 flex items-center gap-2 flex-wrap">
        {(["all","hot","overdue","stale"] as Filter[]).map((f)=>(<button key={f} onClick={()=>setFilter(f)} className={`text-xs px-3 py-1.5 rounded-full border font-medium transition ${filter===f?"bg-red-600 text-white border-red-600":"bg-white text-slate-500 border-slate-200"}`}>{f==="all"&&"All deals"}{f==="hot"&&"🚗 High value"}{f==="overdue"&&"⚠️ Overdue"}{f==="stale"&&"⏳ Stale 7d+"}</button>))}
          ...
          {STAGES.map((stage)=>{const items=stageDeals(stage);const val=stageVal(stage);const isLost=stage==="Closed Lost";return(
            <div key={stage} className="flex flex-col">
              ...
              <div className="flex flex-col gap-3">
                {items.length===0&&<div className="border-2 border-dashed border-slate-200 rounded-xl flex items-center justify-center h-20 text-xs text-slate-400">No deals</div>}
                {items.map((d)=>{const contact=getContact(d.contact_email);const hot=parseMoney(d.arv)>=50000;const stale=isStale(d);const overdue=isOverdue(d.next_follow_up);const dueToday=isDueToday(d.next_follow_up);const expanded=expandedDeal===d.id;const dealNotes=getDealNotes(d.id);return(
                  <div key={d.id} className={`bg-white rounded-xl border text-sm shadow-sm hover:shadow-md transition-all ${hot&&!isLost?"border-red-200":"border-slate-200"} ${isLost?"opacity-60":""}`}>
                    <div className="p-4 cursor-pointer select-none" onClick={()=>setExpandedDeal(expanded?null:d.id!)}>
                      <div className="flex flex-wrap gap-1 mb-2">{hot&&!isLost&&<span className="text-[10px] px-2 py-0.5 rounded-full bg-red-100 text-red-800 font-semibold">🚗 High Value</span>}{stale&&<span className="text-[10px] px-2 py-0.5 rounded-full bg-slate-100 text-slate-500">⏳ Stale</span>}{overdue&&<span className="text-[10px] px-2 py-0.5 rounded-full bg-red-100 text-red-600 font-semibold">⚠️ Overdue</span>}</div>
                      ...
                      <div className="mt-3 grid grid-cols-2 gap-x-4 gap-y-1.5">{[["Vehicle Price",fmt(d.arv)],["Trade-in",fmt(d.offer)],["Commission",fmt(d.amount)],["Customer",d.seller]].map(([label,val])=>(<div key={label}><div className="text-[10px] text-slate-400 uppercase tracking-wide font-semibold">{label}</div><div className="text-xs font-bold text-slate-800 truncate">{val}</div></div>))}</div>
                      ...
                      <div className="mt-3 flex items-center justify-between">{contact?(<div className="flex items-center gap-2"><div className="w-6 h-6 rounded-full bg-red-100 text-red-700 text-[9px] font-bold flex items-center justify-center">{initials(contact.name)}</div><div className="text-xs font-semibold text-slate-700">{contact.name}</div></div>):<span className="text-xs text-slate-400">No contact</span>}<span className="text-[10px] text-slate-400">{dealNotes.length>0?`${dealNotes.length} notes · `:""}{expanded?"▲":"▼"}</span></div>
                    </div>
                    {expanded&&(<div className="border-t border-slate-100 px-4 pb-4 pt-3">
                      ...
                      <div className="grid grid-cols-2 gap-2 mb-4">{(["call","text","testdrive","offer"] as const).map((a)=>(<button key={a} onClick={()=>quickAction(d.id!,a)} className="text-[11px] py-1.5 border border-slate-200 rounded-lg bg-slate-50 hover:bg-slate-100 text-slate-600 font-medium">{a==="call"&&"📞 Call"}{a==="text"&&"💬 Text"}{a==="testdrive"&&"🚗 Test Drive"}{a==="offer"&&"💰 Offer"}</button>))}</div>
                      ...
                      <div className="flex gap-4 mt-3"><button onClick={()=>{openEdit(d);setExpandedDeal(null);}} className="text-xs text-red-600 font-semibold">Edit</button><button onClick={()=>handleDelete(d.id)} className="text-xs text-red-500 font-semibold">Delete</button></div>
                    </div>)}
                  </div>);})}
                {!isLost&&<button onClick={()=>openAdd(stage)} className="text-xs text-slate-400 hover:text-red-600 border-2 border-dashed border-slate-200 hover:border-red-300 rounded-xl py-3 transition">+ Add deal</button>}
              </div>
            </div>);})}
        </div>
      )}
      {panelOpen&&(<div className="fixed inset-0 z-50 flex"><div className="flex-1 bg-black/30" onClick={closePanel}/>
        <div className="w-full max-w-md bg-white h-full shadow-2xl overflow-y-auto flex flex-col">
          <div className="flex items-center justify-between px-6 py-4 border-b border-slate-200 sticky top-0 bg-white"><div className="flex items-center gap-2"><div className="w-6 h-6 rounded bg-red-600 flex items-center justify-center text-white text-[10px] font-bold">AU</div><h2 className="text-base font-bold text-slate-900">{editId!==null?"Edit Deal":"New Auto Deal"}</h2></div><button onClick={closePanel} className="text-slate-400 text-2xl">✕</button></div>
          <form onSubmit={handleSubmit} className="flex-1 px-6 py-5 space-y-4">
            {[{label:"Vehicle / Deal Name *",name:"title",placeholder:"e.g. 2024 Honda Accord EX",required:true},{label:"Customer Name *",name:"seller",placeholder:"Customer full name",required:true},{label:"VIN / Stock #",name:"address",placeholder:"VIN or stock number"}].map((f)=>(<div key={f.name}><label className="block text-xs font-bold text-slate-500 uppercase tracking-wide mb-1">{f.label}</label><input name={f.name} value={(form as any)[f.name]} onChange={handleChange} required={f.required} placeholder={f.placeholder} className="w-full border border-slate-200 rounded-lg px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-red-500"/></div>))}
            <div className="grid grid-cols-2 gap-3">{[["Vehicle Price ($)","arv","e.g. 35000"],["Trade-in ($)","offer","e.g. 8000"]].map(([label,name,ph])=>(<div key={name}><label className="block text-xs font-bold text-slate-500 uppercase tracking-wide mb-1">{label}</label><input name={name} value={(form as any)[name]} onChange={handleChange} placeholder={ph} className="w-full border border-slate-200 rounded-lg px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-red-500"/></div>))}</div>
            <div><label className="block text-xs font-bold text-slate-500 uppercase tracking-wide mb-1">Commission ($)</label><input name="amount" value={form.amount} onChange={handleChange} placeholder="e.g. 2500" className="w-full border border-slate-200 rounded-lg px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-red-500"/></div>
            ...
            <div className="flex gap-3 pt-2 pb-6"><button type="submit" disabled={saving} className="flex-1 bg-red-600 hover:bg-red-700 disabled:opacity-50 text-white font-bold text-sm py-3 rounded-lg">{saving?"Saving...":editId!==null?"Update Deal":"Create Deal"}</button><button type="button" onClick={closePanel} className="px-5 border border-slate-200 text-slate-600 text-sm py-3 rounded-lg">Cancel</button></div>
          </form>
        </div>
      </div>)}
    </div>
