...
const STAGES = ["New Candidate", "Screening", "Submitted", "Interview", "Offer Extended", "Placed", "Closed Lost"];
const STAGE_DOT: Record<string,string> = { "New Candidate":"bg-blue-500","Screening":"bg-violet-500","Submitted":"bg-amber-500","Interview":"bg-orange-500","Offer Extended":"bg-pink-500","Placed":"bg-emerald-500","Closed Lost":"bg-slate-400" };
const EMPTY_FORM = { title:"",address:"",arv:"",offer:"",seller:"",amount:"",stage:"New Candidate",contact_email:"",next_follow_up:"" };
...
export default function RecruitingPipeline(){
  ...
  function openAdd(stage="New Candidate"){setForm({...EMPTY_FORM,stage});setEditId(null);setPanelOpen(true);}
  function openEdit(deal:Deal){setForm({title:deal.title||"",address:deal.address||"",arv:deal.arv||"",offer:deal.offer||"",seller:deal.seller||"",amount:deal.amount||"",stage:deal.stage||"New Candidate",contact_email:deal.contact_email||"",next_follow_up:deal.next_follow_up||""});setEditId(deal.id??null);setPanelOpen(true);}
  ...
  async function handleSubmit(e:React.FormEvent){e.preventDefault();if(!form.title||!form.seller)return;setSaving(true);const user=await getCurrentUser();if(!user){setSaving(false);return;}
    if(editId!==null){const{error}=await supabase.from("deals").update({...form,next_follow_up:form.next_follow_up||null,updated_at:new Date().toISOString()}).eq("id",editId).eq("user_id",user.id);if(error){alert(error.message);setSaving(false);return;}await supabase.from("notes").insert({deal_id:editId,user_id:user.id,content:"✏️ Candidate updated"});}
  ...
  async function quickAction(dealId:number,action:"call"|"email"|"interview"|"offer"){const map={call:"📞 Called candidate",email:"📧 Sent email",interview:"🎯 Interview scheduled",offer:"💼 Offer extended"};await addNote(dealId,map[action]);}
  ...
  const placed=deals.filter((d)=>d.stage==="Placed").length;
  const totalFees=deals.filter((d)=>d.stage==="Placed").reduce((s,d)=>s+parseMoney(d.amount),0);
  ...
  return(
    <div className="min-h-screen bg-slate-50 -m-8">
      <div className="bg-white border-b border-slate-200 px-6 py-3 flex items-center justify-between sticky top-0 z-30">
        <div className="flex items-center gap-3"><div className="w-7 h-7 rounded-lg bg-indigo-600 flex items-center justify-center text-white text-xs font-bold">RC</div><h1 className="text-base font-bold text-slate-900">Recruiting Pipeline</h1><span className="text-slate-300">·</span><span className="text-sm text-slate-400">{deals.length} candidates</span></div>
        <div className="flex items-center gap-2"><input value={search} onChange={(e)=>setSearch(e.target.value)} placeholder="Search candidates..." className="text-sm border border-slate-200 rounded-lg px-3 py-2 w-48 bg-slate-50 focus:outline-none focus:ring-2 focus:ring-indigo-600"/><button onClick={()=>openAdd()} className="bg-indigo-600 hover:bg-indigo-700 text-white text-sm font-semibold px-4 py-2 rounded-lg transition">+ Add Candidate</button></div>
      </div>
      <div className="bg-white border-b border-slate-200 px-6 py-4 grid grid-cols-2 md:grid-cols-4 gap-6">
        {[{label:"Pipeline Salary",value:fmtShort(totalPipeline),sub:`${deals.length} active candidates`},{label:"Placed",value:String(placed),sub:"candidates placed",green:placed>0},{label:"Placement Fees",value:fmtShort(totalFees),sub:"from placements",green:totalFees>0},{label:"Follow-ups Due",value:String(overdueCount),sub:overdueCount>0?"overdue":"all clear ✓",red:overdueCount>0}].map((s)=>(<div key={s.label}><div className="text-xs font-semibold text-slate-400 uppercase tracking-wider">{s.label}</div><div className={`text-2xl font-bold mt-1 ${(s as any).red?"text-red-500":(s as any).green?"text-emerald-600":"text-slate-900"}`}>{s.value}</div><div className={`text-xs mt-0.5 ${(s as any).red?"text-red-400":"text-slate-400"}`}>{s.sub}</div></div>))}
      </div>
      <div className="px-6 py-3 flex items-center gap-2 flex-wrap">
        {(["all","hot","overdue","stale"] as Filter[]).map((f)=>(<button key={f} onClick={()=>setFilter(f)} className={`text-xs px-3 py-1.5 rounded-full border font-medium transition ${filter===f?"bg-indigo-600 text-white border-indigo-600":"bg-white text-slate-500 border-slate-200"}`}>{f==="all"&&"All candidates"}{f==="hot"&&"💼 High salary ($100k+)"}{f==="overdue"&&"⚠️ Overdue"}{f==="stale"&&"⏳ Stale 7d+"}</button>))}
          ...
          {STAGES.map((stage)=>{const items=stageDeals(stage);const val=stageVal(stage);const isLost=stage==="Closed Lost";return(
            <div key={stage} className="flex flex-col">
              ...
              <div className="flex flex-col gap-3">
                {items.length===0&&<div className="border-2 border-dashed border-slate-200 rounded-xl flex items-center justify-center h-20 text-xs text-slate-400">No candidates</div>}
                {items.map((d)=>{const contact=getContact(d.contact_email);const hot=parseMoney(d.arv)>=100000;const stale=isStale(d);const overdue=isOverdue(d.next_follow_up);const dueToday=isDueToday(d.next_follow_up);const expanded=expandedDeal===d.id;const dealNotes=getDealNotes(d.id);return(
                  <div key={d.id} className={`bg-white rounded-xl border text-sm shadow-sm hover:shadow-md transition-all ${hot&&!isLost?"border-indigo-200":"border-slate-200"} ${isLost?"opacity-60":""}`}>
                    <div className="p-4 cursor-pointer select-none" onClick={()=>setExpandedDeal(expanded?null:d.id!)}>
                      <div className="flex flex-wrap gap-1 mb-2">{hot&&!isLost&&<span className="text-[10px] px-2 py-0.5 rounded-full bg-indigo-100 text-indigo-800 font-semibold">💼 High Salary</span>}{stale&&<span className="text-[10px] px-2 py-0.5 rounded-full bg-slate-100 text-slate-500">⏳ Stale</span>}{overdue&&<span className="text-[10px] px-2 py-0.5 rounded-full bg-red-100 text-red-600 font-semibold">⚠️ Overdue</span>}</div>
                      ...
                      <div className="mt-3 grid grid-cols-2 gap-x-4 gap-y-1.5">{[["Salary",fmt(d.arv)],["Role / Dept",d.offer||"—"],["Placement Fee",fmt(d.amount)],["Candidate",d.seller]].map(([label,val])=>(<div key={label}><div className="text-[10px] text-slate-400 uppercase tracking-wide font-semibold">{label}</div><div className="text-xs font-bold text-slate-800 truncate">{val}</div></div>))}</div>
                      ...
                      <div className="mt-3 flex items-center justify-between">{contact?(<div className="flex items-center gap-2"><div className="w-6 h-6 rounded-full bg-indigo-100 text-indigo-700 text-[9px] font-bold flex items-center justify-center">{initials(contact.name)}</div><div className="text-xs font-semibold text-slate-700">{contact.name}</div></div>):<span className="text-xs text-slate-400">No contact</span>}<span className="text-[10px] text-slate-400">{dealNotes.length>0?`${dealNotes.length} notes · `:""}{expanded?"▲":"▼"}</span></div>
                    </div>
                    {expanded&&(<div className="border-t border-slate-100 px-4 pb-4 pt-3">
                      ...
                      <div className="grid grid-cols-2 gap-2 mb-4">{(["call","email","interview","offer"] as const).map((a)=>(<button key={a} onClick={()=>quickAction(d.id!,a)} className="text-[11px] py-1.5 border border-slate-200 rounded-lg bg-slate-50 hover:bg-slate-100 text-slate-600 font-medium">{a==="call"&&"📞 Call"}{a==="email"&&"📧 Email"}{a==="interview"&&"🎯 Interview"}{a==="offer"&&"💼 Offer"}</button>))}</div>
                      ...
                      <div className="flex gap-4 mt-3"><button onClick={()=>{openEdit(d);setExpandedDeal(null);}} className="text-xs text-indigo-600 font-semibold">Edit</button><button onClick={()=>handleDelete(d.id)} className="text-xs text-red-500 font-semibold">Delete</button></div>
                    </div>)}
                  </div>);})}
                {!isLost&&<button onClick={()=>openAdd(stage)} className="text-xs text-slate-400 hover:text-indigo-600 border-2 border-dashed border-slate-200 hover:border-indigo-300 rounded-xl py-3 transition">+ Add candidate</button>}
              </div>
            </div>);})}
        </div>
      )}
      {panelOpen&&(<div className="fixed inset-0 z-50 flex"><div className="flex-1 bg-black/30" onClick={closePanel}/>
        <div className="w-full max-w-md bg-white h-full shadow-2xl overflow-y-auto flex flex-col">
          <div className="flex items-center justify-between px-6 py-4 border-b border-slate-200 sticky top-0 bg-white"><div className="flex items-center gap-2"><div className="w-6 h-6 rounded bg-indigo-600 flex items-center justify-center text-white text-[10px] font-bold">RC</div><h2 className="text-base font-bold text-slate-900">{editId!==null?"Edit Candidate":"New Candidate"}</h2></div><button onClick={closePanel} className="text-slate-400 text-2xl">✕</button></div>
          <form onSubmit={handleSubmit} className="flex-1 px-6 py-5 space-y-4">
            {[{label:"Job Title / Position *",name:"title",placeholder:"e.g. Senior Software Engineer",required:true},{label:"Candidate Name *",name:"seller",placeholder:"Candidate full name",required:true},{label:"Company / Client",name:"address",placeholder:"Hiring company name"}].map((f)=>(<div key={f.name}><label className="block text-xs font-bold text-slate-500 uppercase tracking-wide mb-1">{f.label}</label><input name={f.name} value={(form as any)[f.name]} onChange={handleChange} required={f.required} placeholder={f.placeholder} className="w-full border border-slate-200 rounded-lg px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-600"/></div>))}
            <div className="grid grid-cols-2 gap-3"><div><label className="block text-xs font-bold text-slate-500 uppercase tracking-wide mb-1">Salary ($)</label><input name="arv" value={form.arv} onChange={handleChange} placeholder="e.g. 120000" className="w-full border border-slate-200 rounded-lg px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-600"/></div><div><label className="block text-xs font-bold text-slate-500 uppercase tracking-wide mb-1">Role / Department</label><input name="offer" value={form.offer} onChange={handleChange} placeholder="e.g. Engineering" className="w-full border border-slate-200 rounded-lg px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-600"/></div></div>
            <div><label className="block text-xs font-bold text-slate-500 uppercase tracking-wide mb-1">Placement Fee ($)</label><input name="amount" value={form.amount} onChange={handleChange} placeholder="e.g. 18000" className="w-full border border-slate-200 rounded-lg px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-600"/></div>
            ...
            <div className="flex gap-3 pt-2 pb-6"><button type="submit" disabled={saving} className="flex-1 bg-indigo-600 hover:bg-indigo-700 disabled:opacity-50 text-white font-bold text-sm py-3 rounded-lg">{saving?"Saving...":editId!==null?"Update Candidate":"Create Candidate"}</button><button type="button" onClick={closePanel} className="px-5 border border-slate-200 text-slate-600 text-sm py-3 rounded-lg">Cancel</button></div>
          </form>
        </div>
      </div>)}
    </div>
