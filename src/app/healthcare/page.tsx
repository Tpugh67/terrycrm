...
const STAGES = ["New Patient", "Consultation", "Treatment Plan", "Active Treatment", "Follow-up", "Closed"];
const STAGE_DOT: Record<string,string> = { "New Patient":"bg-blue-500","Consultation":"bg-violet-500","Treatment Plan":"bg-amber-500","Active Treatment":"bg-orange-500","Follow-up":"bg-pink-500","Closed":"bg-emerald-500" };
...
export default function HealthcarePipeline(){
  ...
  async function quickAction(dealId:number,action:"call"|"text"|"appointment"|"followup"){const map={call:"📞 Called patient",text:"💬 Sent text",appointment:"📅 Appointment scheduled",followup:"🏥 Follow-up scheduled"};await addNote(dealId,map[action]);}
  ...
  const activePatients=deals.filter((d)=>d.stage==="Active Treatment").length;
  return(
    <div className="min-h-screen bg-slate-50 -m-8">
      <div className="bg-white border-b border-slate-200 px-6 py-3 flex items-center justify-between sticky top-0 z-30">
        <div className="flex items-center gap-3"><div className="w-7 h-7 rounded-lg bg-cyan-600 flex items-center justify-center text-white text-xs font-bold">HC</div><h1 className="text-base font-bold text-slate-900">Healthcare Pipeline</h1><span className="text-slate-300">·</span><span className="text-sm text-slate-400">{deals.length} patients</span></div>
        <div className="flex items-center gap-2"><input value={search} onChange={(e)=>setSearch(e.target.value)} placeholder="Search patients..." className="text-sm border border-slate-200 rounded-lg px-3 py-2 w-48 bg-slate-50 focus:outline-none focus:ring-2 focus:ring-cyan-600"/><button onClick={()=>openAdd()} className="bg-cyan-600 hover:bg-cyan-700 text-white text-sm font-semibold px-4 py-2 rounded-lg transition">+ Add Patient</button></div>
      </div>
      <div className="bg-white border-b border-slate-200 px-6 py-4 grid grid-cols-2 md:grid-cols-4 gap-6">
        {[{label:"Pipeline Value",value:fmtShort(totalPipeline),sub:`${deals.length} total patients`},{label:"Closed Revenue",value:fmtShort(closedRevenue),sub:"completed treatments",green:true},{label:"Active Patients",value:String(activePatients),sub:"in active treatment",green:activePatients>0},{label:"Follow-ups Due",value:String(overdueCount),sub:overdueCount>0?"overdue":"all clear",red:overdueCount>0}].map((s)=>(<div key={s.label}><div className="text-xs font-semibold text-slate-400 uppercase tracking-wider">{s.label}</div><div className={`text-2xl font-bold mt-1 ${(s as any).red?"text-red-500":(s as any).green?"text-emerald-600":"text-slate-900"}`}>{s.value}</div><div className={`text-xs mt-0.5 ${(s as any).red?"text-red-400":"text-slate-400"}`}>{s.sub}</div></div>))}
      </div>
      <div className="px-6 py-3 flex items-center gap-2 flex-wrap">
        {(["all","hot","overdue","stale"] as Filter[]).map((f)=>(<button key={f} onClick={()=>setFilter(f)} className={`text-xs px-3 py-1.5 rounded-full border font-medium transition ${filter===f?"bg-cyan-600 text-white border-cyan-600":"bg-white text-slate-500 border-slate-200"}`}>{f==="all"&&"All patients"}{f==="hot"&&"💎 High value"}{f==="overdue"&&"⚠️ Overdue"}{f==="stale"&&"⏳ Stale 7d+"}</button>))}
                ...
                {items.map((d)=>{const contact=getContact(d.contact_email);const hot=parseMoney(d.arv)>=5000;const stale=isStale(d);const overdue=isOverdue(d.next_follow_up);const dueToday=isDueToday(d.next_follow_up);const expanded=expandedDeal===d.id;const dealNotes=getDealNotes(d.id);return(
                  <div key={d.id} className={`bg-white rounded-xl border text-sm shadow-sm hover:shadow-md transition-all ${hot?"border-cyan-200":"border-slate-200"}`}>
                    <div className="p-4 cursor-pointer select-none" onClick={()=>setExpandedDeal(expanded?null:d.id!)}>
                      ...
                      <div className="mt-3 grid grid-cols-2 gap-x-4 gap-y-1.5">{[["Treatment Value",fmt(d.arv)],["Insurance",d.offer||"—"],["Co-pay",fmt(d.amount)],["Patient",d.seller]].map(([label,val])=>(<div key={label}><div className="text-[10px] text-slate-400 uppercase tracking-wide font-semibold">{label}</div><div className="text-xs font-bold text-slate-800 truncate">{val}</div></div>))}</div>
                      ...
                      <div className="mt-3 flex items-center justify-between">{contact?(<div className="flex items-center gap-2"><div className="w-6 h-6 rounded-full bg-cyan-100 text-cyan-700 text-[9px] font-bold flex items-center justify-center">{initials(contact.name)}</div><div className="text-xs font-semibold text-slate-700">{contact.name}</div></div>):<span className="text-xs text-slate-400">No contact</span>}<span className="text-[10px] text-slate-400">{dealNotes.length>0?`${dealNotes.length} notes · `:""}{expanded?"▲":"▼"}</span></div>
                    </div>
                    {expanded&&(<div className="border-t border-slate-100 px-4 pb-4 pt-3">
                      ...
                      <div className="grid grid-cols-2 gap-2 mb-4">{(["call","text","appointment","followup"] as const).map((a)=>(<button key={a} onClick={()=>quickAction(d.id!,a)} className="text-[11px] py-1.5 border border-slate-200 rounded-lg bg-slate-50 hover:bg-slate-100 text-slate-600 font-medium">{a==="call"&&"📞 Call"}{a==="text"&&"💬 Text"}{a==="appointment"&&"📅 Appt"}{a==="followup"&&"🏥 Follow-up"}</button>))}</div>
                      ...
                      <div className="flex gap-4 mt-3"><button onClick={()=>{openEdit(d);setExpandedDeal(null);}} className="text-xs text-cyan-600 font-semibold">Edit</button><button onClick={()=>handleDelete(d.id)} className="text-xs text-red-500 font-semibold">Delete</button></div>
                    </div>)}
                  </div>);})}
                <button onClick={()=>openAdd(stage)} className="text-xs text-slate-400 hover:text-cyan-600 border-2 border-dashed border-slate-200 hover:border-cyan-300 rounded-xl py-3 transition">+ Add patient</button>
              </div>
            </div>);})}
        </div>
      )}
      {panelOpen&&(<div className="fixed inset-0 z-50 flex"><div className="flex-1 bg-black/30" onClick={closePanel}/>
        <div className="w-full max-w-md bg-white h-full shadow-2xl overflow-y-auto flex flex-col">
          <div className="flex items-center justify-between px-6 py-4 border-b border-slate-200 sticky top-0 bg-white"><div className="flex items-center gap-2"><div className="w-6 h-6 rounded bg-cyan-600 flex items-center justify-center text-white text-[10px] font-bold">HC</div><h2 className="text-base font-bold text-slate-900">{editId!==null?"Edit Patient":"New Patient"}</h2></div><button onClick={closePanel} className="text-slate-400 text-2xl">✕</button></div>
          <form onSubmit={handleSubmit} className="flex-1 px-6 py-5 space-y-4">
            {[{label:"Treatment / Case Name *",name:"title",placeholder:"e.g. Smith - Knee Replacement",required:true},{label:"Patient Name *",name:"seller",placeholder:"Patient full name",required:true},{label:"Referring Doctor / Source",name:"address",placeholder:"e.g. Dr. Johnson referral"}].map((f)=>(<div key={f.name}><label className="block text-xs font-bold text-slate-500 uppercase tracking-wide mb-1">{f.label}</label><input name={f.name} value={(form as any)[f.name]} onChange={handleChange} required={f.required} placeholder={f.placeholder} className="w-full border border-slate-200 rounded-lg px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-cyan-600"/></div>))}
            <div className="grid grid-cols-2 gap-3"><div><label className="block text-xs font-bold text-slate-500 uppercase tracking-wide mb-1">Treatment Value ($)</label><input name="arv" value={form.arv} onChange={handleChange} placeholder="e.g. 8000" className="w-full border border-slate-200 rounded-lg px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-cyan-600"/></div><div><label className="block text-xs font-bold text-slate-500 uppercase tracking-wide mb-1">Insurance Provider</label><input name="offer" value={form.offer} onChange={handleChange} placeholder="e.g. Blue Cross" className="w-full border border-slate-200 rounded-lg px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-cyan-600"/></div></div>
            <div><label className="block text-xs font-bold text-slate-500 uppercase tracking-wide mb-1">Co-pay / Out of Pocket ($)</label><input name="amount" value={form.amount} onChange={handleChange} placeholder="e.g. 1500" className="w-full border border-slate-200 rounded-lg px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-cyan-600"/></div>
            ...
            <div className="flex gap-3 pt-2 pb-6"><button type="submit" disabled={saving} className="flex-1 bg-cyan-600 hover:bg-cyan-700 disabled:opacity-50 text-white font-bold text-sm py-3 rounded-lg">{saving?"Saving...":editId!==null?"Update Patient":"Add Patient"}</button><button type="button" onClick={closePanel} className="px-5 border border-slate-200 text-slate-600 text-sm py-3 rounded-lg">Cancel</button></div>
          </form>
        </div>
      </div>)}
    </div>
